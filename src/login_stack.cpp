...
#include "window.h"
#include "globals.h"
LoginStack::LoginStack(const Glib::RefPtr<Gtk::Builder>& builder, Window *f):father(f)
{
    //Entrada de usuario
    ...
    IN_IDEN->signal_activate().connect(sigc::mem_fun(*this, &LoginStack::on_IN_IDEN_activate));
...
void LoginStack::on_IN_IDEN_activate()
{
...
void LoginStack::NfcDetected(const std::string  in){
