...
const litesql::FieldType Key::Pub("pub_",A_field_type_boolean,table__);
void Key::initValues() {
}
void Key::defaults() {
    id = 0;
...
Key::Key(const litesql::Database& db)
     : litesql::Persistent(db), id(Id), type(Type), name(Name), ubi(Ubi), commentary(Commentary), pos(Pos), active(Active), uid(Uid), pub(Pub) {
    defaults();
}
Key::Key(const litesql::Database& db, const litesql::Record& rec)
     : litesql::Persistent(db, rec), id(Id), type(Type), name(Name), ubi(Ubi), commentary(Commentary), pos(Pos), active(Active), uid(Uid), pub(Pub) {
    defaults();
    size_t size = (rec.size() > 9) ? 9 : rec.size();
    switch(size) {
    case 9: pub = convert<const std::string&, bool>(rec[8]);
...
Key::Key(const Key& obj)
     : litesql::Persistent(obj), id(obj.id), type(obj.type), name(obj.name), ubi(obj.ubi), commentary(obj.commentary), pos(obj.pos), active(obj.active), uid(obj.uid), pub(obj.pub) {
}
...
std::string Key::insert(litesql::Record& tables, litesql::Records& fieldRecs, litesql::Records& valueRecs) {
    ...
    values.push_back(uid);
    uid.setModified(false);
    fields.push_back(pub.name());
    values.push_back(pub);
    pub.setModified(false);
    fieldRecs.push_back(fields);
    valueRecs.push_back(values);
    return litesql::Persistent::insert(tables, fieldRecs, valueRecs, sequence__);
...
void Key::addUpdates(Updates& updates) {
    ...
    updateField(updates, table__, uid);
    updateField(updates, table__, pub);
}
