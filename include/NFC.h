#ifndef NFC_H_INCLUDED
#define NFC_H_INCLUDED
...
size_t build_ndef_text_record(const char *text, uint8_t *buffer, size_t buffer_size) {
    size_t text_len = strlen(text);
    size_t lang_len = strlen(LANG_CODE);

    // Longitud del payload = 1 (status byte) + lang_len + text_len
    size_t payload_len = 1 + lang_len + text_len;

    // Mensaje mínimo = header + type length + payload length + type + payload
    size_t message_len = 3 + 1 + payload_len;

    if (message_len > buffer_size) {
        return 0; // Error: no cabe
    }

    size_t offset = 0;

    // Header: MB=1, ME=1, SR=1, TNF=0x1 (well-known type)
    buffer[offset++] = 0xD1;
    ...
    // Status byte: UTF-8 (bit7=0) + lang_len
    buffer[offset++] = (uint8_t)lang_len;

    // Language code
    memcpy(&buffer[offset], LANG_CODE, lang_len);
    offset += lang_len;
    ...
    return offset; // longitud final del mensaje
}
...
write_nfg(int argc, char *argv[])
{
    ...
    MifareClassicKey transport_key = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };
	 ...
	 ndef_msg_len = build_ndef_text_record(" y feina", ndef_default_msg, sizeof(ndef_default_msg));
